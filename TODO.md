...
- [ ] Create GitHub repository `smallaxe`
- [ ] Clone locally
- [ ] Create `.gitignore` (Python, IDE, Spark, etc.)
...
#### Step 0.3: Package Configuration (pyproject.toml)
- [ ] Create `pyproject.toml` with:
  - Package metadata (name, version=0.1.0, description, author)
  - Python requires: >=3.8,<3.11
  - Core dependencies: pyspark>=3.3, hyperopt, plotly
  - Optional dependencies: [xgboost], [lightgbm], [catboost], [all], [dev]
  - Dev dependencies: pytest, pytest-spark, black, ruff, mypy
- [ ] Create `requirements-dev.txt` for local development
- [ ] Verify: `pip install -e .` works
- [ ] Commit: "Add pyproject.toml and package configuration"
- [ ] PR → main

#### Step 0.4: GitHub Actions CI/CD
- [ ] Create `.github/workflows/ci.yml`:
  - Trigger on PR to main: Python 3.10 + Spark 3.5 only (fast feedback)
  - Trigger on push to main: Full matrix Python [3.8, 3.9, 3.10] × Spark [3.3, 3.4, 3.5]
  - Steps: checkout, setup Python, setup Java (for Spark), install deps, run pytest
- [ ] Create `.github/workflows/publish.yml` (for PyPI, triggered on release)
- [ ] Add test workflow badge to README
- [ ] Commit: "Add GitHub Actions CI/CD workflows"
- [ ] PR → main

#### Step 0.5: Testing Infrastructure
- [ ] Create `tests/conftest.py` with PySpark fixtures:
  - `spark_session` fixture (session-scoped)
  - `sample_df` fixture for basic testing
- [ ] Create `tests/test_smoke.py` with basic import test
- [ ] Run locally: `pytest tests/ -v`
- [ ] Commit: "Add pytest infrastructure and smoke test"
- [ ] PR → main

---

### Phase 1: Exceptions Module (v0.1.0)

#### Step 1.1: Exception Classes
- [ ] Create `smallaxe/exceptions/__init__.py`
- [ ] Implement exception hierarchy:
  ...
  ```
- [ ] Each exception should have informative default messages
- [ ] Create `tests/test_exceptions.py`:
  - Test each exception can be raised and caught
  - Test exception inheritance
  - Test custom messages
- [ ] Run locally: `pytest tests/test_exceptions.py -v`
- [ ] Commit: "Add custom exception classes"
- [ ] PR → main

---

### Phase 2: Configuration Module (v0.1.0)

#### Step 2.1: Verbosity and Config
- [ ] Create `smallaxe/_config.py` (internal config state)
- [ ] Update `smallaxe/__init__.py` with:
  ```python
  ...
  def get_seed() -> int: ...
  def set_cache_strategy(strategy: str) -> None: ...  # 'auto', 'always', 'never'
  def get_cache_strategy() -> str: ...
  ```
- [ ] Implement verbosity levels: 'quiet', 'normal', 'verbose'
- [ ] Implement cache strategies: 'auto', 'always', 'never'
- [ ] Raise `ConfigurationError` for invalid values
- [ ] Create `tests/test_config.py`:
  - Test set/get verbosity
  - Test verbosity context manager
  - Test invalid verbosity raises ConfigurationError
  - Test spark session set/get
  - Test set/get seed
  - Test set/get cache_strategy
- [ ] Run locally: `pytest tests/test_config.py -v`
- [ ] Commit: "Add configuration and verbosity management"
- [ ] PR → main
- [ ] **Tag: v0.1.0**

---

### Phase 3: Sample Datasets (v0.2.0)

#### Step 3.1: Regression Dataset
- [ ] Create `smallaxe/datasets/_data.py` with raw data generators
- [ ] Implement `load_sample_regression(spark)`:
  - Generate ~10,000 rows of housing data
  - Columns: bedrooms, bathrooms, sqft, age, location, condition, price
  - Realistic distributions and correlations
- [ ] Create `tests/test_datasets.py`:
  - Test dataset loads successfully
  - Test correct schema (column names, types)
  - Test row count
  - Test no null values
- [ ] Run locally: `pytest tests/test_datasets.py -v`
- [ ] Commit: "Add sample regression dataset"
- [ ] PR → main

#### Step 3.2: Classification Dataset
- [ ] Implement `load_sample_classification(spark)`:
  - Generate ~10,000 rows of churn data
  - Columns: tenure, monthly_charges, total_charges, contract, payment_method, churn
  - Balanced classes (~30% churn)
- [ ] Implement `dataset_info(dataset_name)` function
- [ ] Update `tests/test_datasets.py`:
  - Test classification dataset schema
  - Test class balance
  - Test dataset_info output
- [ ] Export functions in `smallaxe/datasets/__init__.py`
- [ ] Run locally: `pytest tests/test_datasets.py -v`
- [ ] Commit: "Add sample classification dataset and dataset_info"
- [ ] PR → main
- [ ] **Tag: v0.2.0**
